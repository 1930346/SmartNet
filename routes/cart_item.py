...
@cart_item.get("/cart_items", response_model=list[Cart_item], tags = ["cart_items"])
def get_cart_items():
    return conn.execute(cart_item.select()).fetchall()


#Obtención de un cart_item por id
@cart_item.get("/cart_items/{id}", response_model=Cart_item, tags = ["cart_items"])
def get_cart_item(id: str):
    return conn.execute(cart_item.select().where(cart_item.c.id == id)).first()


#Creación de un cart_item
@cart_item.post("/cart_items", response_model=Cart_item, tags = ["cart_items"])
def create_cart_item(cart_item: Cart_item):
    new_cart_item = {
...
@cart_item.put("/cart_items/{id}", response_model=Cart_item, tags = ["cart_items"])
def update_cart_item(id: str, cart_item: Cart_item):
